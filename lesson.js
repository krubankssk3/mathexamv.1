 ...
 *
 * + โหมดเสริม "สร้างแบบฝึกการบวก (ตั้งบวก)":
 *   ตัวตั้ง/ตัวบวก ตั้งหลักได้ 1–8 หลัก · A4 แนวตั้ง ตารางช่องตัวเลข
 *   เฉลย (inline/พิมพ์พร้อมเฉลย/QR มุมขวา) · จับเวลาเต็มจอ · ช่อง "เวลาที่ใช้"
 ***********************************************************************/
(function () {
  ...
  function addPrintCSS() {
    return ''
      ...
      + '.hd .ttl{font-size:20px;font-weight:700;color:#c0392b;line-height:1.1}'
      + '.hd .nm{font-size:14px;margin-top:6px;padding-right:96px}'
      + '.hd .meta{display:flex;flex-wrap:wrap;gap:6px 18px;font-size:13px;margin-top:7px;align-items:center}'
  ...
  window.Platform.register({
    ...
    mount: function (host, svc) {
      ...
      /* เปิดตามโหมดเริ่มต้น */
      if (st.mode === 'add') renderAdd(); else renderList();
    }
  });
})();
