        ...
        svc.swal.fire(Object.assign({
          ...
          html:
            '<div style="text-align:left">' +
              ...
              '<select id="sw_cg" style="' + INP + '">' + genOptions(editing ? c.gen : '') + '</select>' +
          ...
          preConfirm: function () {
            ...
            return { chapterName: name, icon: (document.getElementById('sw_ci').value || 'ti-file').trim(), gen: document.getElementById('sw_cg').value, ops: (document.getElementById('sw_co').value || '').trim() };
          }
          ...
          var call = editing
            ? svc.api('updateChapter', { chapterId: c.id, chapterName: p.chapterName, icon: p.icon, gen: p.gen, ops: p.ops })
            : svc.api('addChapter', { gradeId: g.id, chapterName: p.chapterName, icon: p.icon, gen: p.gen, ops: p.ops });
          call.then(function () { svc.done(); svc.toast(editing ? 'แก้ไขแล้ว' : 'เพิ่มบทเรียนแล้ว', 'success'); load(); })
            .catch(function (e) { svc.done(); svc.toast(String(e), 'error'); });
        });
      }
