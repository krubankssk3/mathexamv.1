  ...
  window.Platform.register({
    ...
    mount: function (host, svc) {
      ...
      var DEF_INSTR = {
        ...
        measlen: 'ดูรูปสิ่งของที่วางบนไม้บรรทัด แล้วบอกความยาวออกมาเป็นเซนติเมตรและมิลลิเมตร',
        ...
        pattern: 'เติมจำนวนที่หายไปในแบบรูป และบอกว่าเป็นแบบรูปที่เพิ่มขึ้นหรือลดลง ทีละเท่าใด',
        numline: 'แสดงวิธีหาผลลัพธ์โดยใช้เส้นจำนวน แล้วเขียนคำตอบลงในช่องว่าง'
      };
      function makeSet() {
        var g = gradeOf(st.gradeId), ch = chapterOf(st.gradeId, st.chapterId);
        ...
        if (ch.gen === 'numline' && ch.lv && !(ch.lv.instruction)) {
          var _no = ch.lv.op === '-' ? 'ผลลบ' : (ch.lv.op === 'mix' ? 'ผลบวกหรือผลลบ' : 'ผลบวก');
          instrText = (ch.lv.layout === 'blank')
            ? ('เขียนแสดงวิธีหา' + _no + 'บนเส้นจำนวน แล้วเติมคำตอบ')
            : ('แสดงวิธีหา' + _no + 'โดยใช้เส้นจำนวน แล้วเติมจำนวนลงในช่องว่างให้ถูกต้อง');
        }
        if (ch.gen === 'pattern' && ch.lv && !(ch.lv.instruction)) {
          var _ps = ch.lv.step || 2;
          if (ch.lv.layout === 'mark') instrText = 'เขียน ✓ ในช่องที่เป็นแบบรูปของจำนวนที่เพิ่มขึ้นทีละ ' + _ps + ' และเขียน ✗ ในช่องที่เป็นแบบรูปของจำนวนที่ลดลงทีละ ' + _ps;
          else instrText = 'เติมจำนวนที่หายไปในแบบรูป แล้วเติมว่าเป็นแบบรูปที่เพิ่มขึ้นหรือลดลง และทีละเท่าใด';
        }
